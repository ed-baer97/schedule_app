...
3. **Настройка сервиса**
   ...
   - Установка должна проходить быстро, так как тяжелые пакеты удалены
...
- Build command использует `--prefer-binary` для установки предкомпилированных wheels где возможно
- Установка должна проходить быстро, так как тяжелые пакеты (ortools, deap) удалены
- Если все равно видите "Preparing metadata", это нормально для некоторых пакетов (numpy, pandas)
